...
> - 求解最优解：$$\alpha^{*}=(\alpha_{1}^{*},\alpha_{2}^{*},...,\alpha_{N}^{*})$$
> - 计算  $\omega^{*},b^{*}$
> $$\omega^{*}=\sum_{i=1}^{N}\alpha_{i}^{*}y_ix_i;$$
> - 选择 $\alpha^{*}$的一个正分量 $\alpha_{j}^{*}$，计算
> $$b^{*}=y_j-\sum_{i=1}^{N}\alpha_{i}^{*}y_i(x_i\cdot x_j)$$
> - 求出分离超平面和分类决策函数,
>     - 分离平面：$$\omega^{*}\cdot x_i + b^{*}=0 $$
>     - 决策函数：$$f(x)=sign(\omega^*\cdot x+b^*)$$


定义(支持向量)： 从上面可以看出，$\omega^{*}$  和 $b^{*}$ 只依赖于训练数据中 $\alpha_{i}^{*} > 0$ 的点，我们将训练数据中对应 $\alpha_{i}^{*} > 0$ (互补松弛条件可知，此时有 $\omega^{*}\cdot x_i + b^{*}=1$ )的实例点 ($\alpha_i^*\in R^n$) 称为支持向量。
